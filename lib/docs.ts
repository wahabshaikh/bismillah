...
export const DOCS: DocPage[] = [
  ...
  },
  {
    ...
    body: [
      ...
      "The `/api/auth/[...all]` route is rate-limited via `lib/rate-limit.ts` using Cloudflare's native Rate Limiting binding. Add an auth-gated page by calling `const { user } = await requireSession()` at the top of the RSC and `export const dynamic = \"force-dynamic\"`.",
    ],
  },
  {
  ...
  },
  {
    ...
    body: [
      ...
      "When `PRODUCT_API_KEY` is set, `/api/v1/notes` requires `Authorization: Bearer <key>` (health stays open). When it is unset the whole surface is open for local demos. Writes use native rate limiting. All routes send permissive CORS headers and answer `OPTIONS` preflight.",
  ...
  },
  {
    ...
    body: [
      "The wrangler binding IDs are load-bearing. Never overwrite them, and never merge the two KV namespaces — `KV` is demo app data, `VINEXT_KV_CACHE` is reserved for vinext's ISR/data cache, and request throttling uses `RATE_LIMITER_*`.",
      "- DB — D1 `bismillah` — 36a267af-1bd7-4da0-af68-95654c6d46c3",
      "- ARTIFACTS — R2 bucket `bismillah-artifacts`",
      "- KV — 2423ae5f6f90406d8a2233680c5d1c26 (demo app data only)",
